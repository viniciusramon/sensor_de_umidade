...
int main()
{
	...
	/* Enable clock and init GPIO outputs */
		...
		GPIOE_AHB->PDR   		    |=  PE0;  								      /* pull-down PE0         */
  	...
  	/* SWITCH */
		...
		GPIOA_AHB->PDR   		    |=  PA6;  								      /* pull-down PA6          */
		...
		/* SERIAL */
			...
			switch(referencia)
			{
				case 0:
					...
					sprintf(SValue,"%.2f",(float)set_max/100);
					LCD_Out(SValue,2,2);
				  LCD_Out("V:",2,0);
			...
			break;
				...
				case 1:
					...
					sprintf(SValue,"%.2f",(float)set_min/100);
					LCD_Out(SValue,2,2);
					LCD_Out("V:",2,0);
			...
			if ((AVoltage) >= (ref_max + 10))
				{
					LCD_Out("Seco      ",1,0);
				}
				
				else if (((AVoltage) <= (ref_max + 10) && ((AVoltage) >= (ref_min - 10))))
				{
					LCD_Out("Ideal     ",1,0);
				}
				
				else if ((AVoltage) <= (ref_min - 10))
				{
